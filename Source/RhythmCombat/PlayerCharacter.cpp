		...
		case BasicAttack:
			...
			for (int i = 0; i < TargetArray[0].OwningChar->TargetList.Num(); i++) {
				...
				TargetArray[0].OwningChar->TargetList[i]->CharacterStats.HealthPoints -=
					TargetArray[0].OwningChar->Level * 2 * accuracyaverage / 100 * TargetArray[0].OwningChar->TargetList[i]->DefenseModifier;
...
void APlayerCharacter::NavigateUp() {
	...
	if (CombatManagerRef->TargetCategory == Ally) {
		...
		if (CombatManagerRef->SelectedTarget == CombatManagerRef->EnemyParty[0] && CombatManagerRef->EnemyParty.Num() > 1) {
...
void APlayerCharacter::NavigateDown() {
	...
	if (CombatManagerRef->TargetCategory == Ally) {
		...
		if (CombatManagerRef->SelectedTarget == CombatManagerRef->EnemyParty[CombatManagerRef->EnemyParty.Num() - 1] && CombatManagerRef->EnemyParty.Num() > 1) {
